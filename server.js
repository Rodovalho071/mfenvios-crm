...
function saveMessages(msgs) { fs.writeFileSync(DATA_FILE,JSON.stringify(msgs)); }
function loadKanban() {
...
function saveKanban(data) { fs.writeFileSync(KANBAN_FILE,JSON.stringify(data)); }
const sseClients = [];
...
app.post('/webhook',(req,res) => {
  ...
  if(b.event==='messages.upsert'&&b.data) {
    ...
    if(m.key&&!m.key.fromMe) {
      ...
      const msg={ id:m.key.id, name:m.pushName||'Desconhecido', phone:cleanPhone(m.key.remoteJid), text:m.message?.conversation||m.message?.extendedTextMessage?.text||'[midia]', ts:Date.now() };
...
app.delete('/messages/:id',(req,res) => {
  ...
  saveMessages(msgs); res.json({ok:true});
});
app.post('/funil',(req,res) => {
  ...
  if(!jaExiste) {
    kanban.cards.unshift({id:Date.now().toString(),name:msg.name,phone:msg.phone,coluna:kanban.colunas[0],ts:Date.now(),mensagens:[msg.text]});
    saveKanban(kanban);
    broadcast('novo-lead',kanban.cards[0]);
  }
  res.json({ok:true,kanban});
});
app.get('/kanban',(req,res) => { res.json(loadKanban()); });
...
app.listen(3000,'0.0.0.0',()=>console.log('Servidor MF CRM rodando na porta 3000'));