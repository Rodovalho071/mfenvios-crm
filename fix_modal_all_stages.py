...
old = """  // Usar etapas do perfil do lead (ou do perfil ativo)
...
new = """  // Mostrar todas as etapas dos dois perfis juntas
  var _allStages=[...new Set([...PERFIS.beatriz.stages,...PERFIS.davi.stages])];
  if(l.stage && !_allStages.includes(l.stage)) _allStages=[l.stage,..._allStages];
  const stageOpts=_allStages.map(s=>`<option value="${s}" ${s===l.stage?'selected':''}>${s}</option>`).join('');"""

if old in html:
    html = html.replace(old, new)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('OK - todas as etapas aparecem no modal')
    print('git add index.html && git commit -m "fix: todas etapas no modal" && git push origin main')
else:
