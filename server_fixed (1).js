...
async function leadExistePorPhone(phone, perfil) {
  if(kanbanCol) {
    // Busca por 'tel' ou 'phone', e 'agent' ou 'perfil'
    const found = await kanbanCol.findOne({
      $and: [
        { $or: [ {tel: phone}, {phone: phone} ] },
        { $or: [ {agent: perfil}, {perfil: perfil} ] }
      ]
    });
    return !!found;
  }
  return memKanban.cards.some(c => (c.tel === phone || c.phone === phone) && (c.agent === perfil || c.perfil === perfil));
}
